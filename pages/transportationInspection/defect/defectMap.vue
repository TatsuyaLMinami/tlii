<template>
  <div style="height:100%;width:100%;">
    <div id="defectMap" :style="style">
      <div class="epgis-control-container">
        <div class="bottom-right">
          <div class="block">
            <div class="title">缺陷等级统计</div>
            <div
              class="content"
            ...
            >
              ...
              <div class="num" style="width: 50%">
                {{ item.natureDefectNumber }}
              </div>
            </div>
          </div>
          <div class="block">
            <div class="title">消缺状态统计</div>
            <div
              class="content"
            ...
            >
              <div class="des" style="width: 50%">
                {{ item.isCompletedName }}
              </div>
              <div class="num" style="width: 20%">
                {{ item.stateDefectNumber }}
              </div>
...
import mapData from "@/store/mapItems/defectData.js";
export default {
  name: "defectMap",
  data() {
    return {
  ...
  methods: {
    setData() {
      const url = "/yangkai/ptmidefectrecord/defectShow";
      $axios({ url: url, type: "post", data: {} })
        .then(data => {
          if (data.successful) {
            for (let i = 0; i < data.resultValue.stateTotal.length; i++) {
              for (let key in this.mapData) {
                if (
                  data.resultValue.stateTotal[i]["isCompleted"] ===
                  this.mapData[key]["isCompleted"]
                ) {
                  data.resultValue.stateTotal[i] = {
                    ...data.resultValue.stateTotal[i],
                    ...this.mapData[key]
                  };
                }
            ...
            for (let i = 0; i < data.resultValue.list.length; i++) {
              for (let key in this.mapData) {
                if (
                  data.resultValue.list[i]["isCompleted"] ===
            ...
            this.stateTotal = data.resultValue.stateTotal;
            this.natureTotal = data.resultValue.natureTotal;
            this.drawEpgis();
          } else {
        ...
        epgis.tokenTask
          ...
          .then(function() {
            that.map = new epgis.Map({
              container: "defectMap", //标准底图使用的是矢量切片，样式美观可定制，但是渲染性能消耗较多，暂时只推荐pc端使用
            ...
            for (let key in that.mapData) {
              ...
              that.map.on("click", that.mapData[key].id, function(e) {
                ...
                that.getPic(row["objId"]).then(data => {
                  ...
                  that.$refs.detail.setData("缺陷详情", { list: arr });
                });
              });
            }
          });
    ...
    //添加缺陷图层
    addDefectLayer(id, features, imageName) {
      let vm = this;
