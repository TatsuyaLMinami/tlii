...
//				me.trigger("saving", args);
		...
		me.getChangedItems = function() {
			...
			for (var i = 0; i < me._changedItems.length; i++) {
		...
		me._parseData = function(p_result) {
			if (me.type == "form") {
		        ...
		        if (cube.isEmpty(me.data) || (cube.isArray(me.data) && me.data.length == 0)) {
		            me.data = new Object();
		        }
		...
		me._parseDicts = function(p_result) {
			...
			if (cube.isArray(me.dicts)) {
				for (var i = 0; i < me.dicts.length; i++) {
					var dict = me.dicts[i];
					if (cube.isObject(dict) && typeof(dict.name) != "undefined" && typeof(dict.values) != "undefined") {
		...
		me._showError = function(p_msgKey, p_context) {
			...
			if (p_context.type != "error") {
				if (p_context.type == "warn" || p_context.type == "info") {
					...
					if (me._viewModel) {
						cube.indicate.call(me._viewModel, "error", cube.msg("REQUEST_TIMEOUT"));
					}else {
